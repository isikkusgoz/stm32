...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define LOGICAL_ONE 	14
#define LOGICAL_ZERO 	7
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void reset_all_leds(rgb_led* leds,uint16_t number_leds)
{
...
void set_all_leds(rgb_led* leds,uint16_t number_leds)
{
	for(int i=0; i < number_leds; i++)
	{
		for(int j=0; j < 8; j++)
		{
			(leds + i) ->r[j] = LOGICAL_ONE;
			(leds + i) ->g[j] = LOGICAL_ONE;
			(leds + i) ->b[j] = LOGICAL_ONE;
		}
	}
}
void set_specific_led(rgb_led* leds, uint16_t led_position, rgb_color color)
{
	for(int j=0; j < 8; j++)
	{
		if(color.r & 0x1 <<j)
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  //3 adet deneme kodu var istediğiniz kodları commente alarak kullanabilirsiniz.

//	  //1. ledlerin hepsini yakıp söndürme.
//	  reset_all_leds(leds, NUMBER_OF_LEDS);
//	  HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t *)leds, NUMBER_OF_LEDS * 24 + 24);
//	  HAL_Delay(300);
//	  set_all_leds(leds, NUMBER_OF_LEDS);
//	  HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t *)leds, NUMBER_OF_LEDS * 24 + 24);
//	  HAL_Delay(300);

//	  //2. kara şimşek tarzı kayan kırmızı led.
...
//	  HAL_Delay(70);
	  ...
	  set_specific_led(leds, 4, blue_color);
	  set_specific_led(leds, 5, blue_color);
	  set_specific_led(leds, 6, blue_color);
	  set_specific_led(leds, 7, blue_color);
	  HAL_TIM_PWM_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t *)leds, NUMBER_OF_LEDS * 24 + 24);
	  HAL_Delay(100);
